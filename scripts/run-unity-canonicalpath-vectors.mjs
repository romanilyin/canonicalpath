...
const tempRoot = path.join(root, "tmp", "unity-canonicalpath-vector-check");
const projectPath = path.join(tempRoot, "CanonicalPath.Unity.VectorCheck.csproj");
...
if (!dotnet) {
  console.log("dotnet not found; skipping Unity CanonicalPath C# vector check");
  process.exit(0);
}
...
const result = spawnSync(
  dotnet.command,
  [...dotnet.prefixArgs, "run", "--project", dotnet.nativePath(projectPath), "--", ...vectorFiles.map((file) => dotnet.nativePath(file))],
  { stdio: "inherit" },
);
...
internal static class Program
{
    private static int Main(string[] args)
    {
        ...
        Console.WriteLine("Unity CanonicalPath C# vectors passed: {0} cases", count);
        return 0;
    }
