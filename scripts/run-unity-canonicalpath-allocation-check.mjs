...
const tempRoot = path.join(root, "tmp", "unity-canonicalpath-allocation-check");
const projectPath = path.join(tempRoot, "CanonicalPath.Unity.AllocationCheck.csproj");
const programPath = path.join(tempRoot, "Program.cs");

const dotnet = findDotnet();
if (!dotnet) {
  console.log("dotnet not found; skipping Unity CanonicalPath C# allocation check");
  process.exit(0);
}
...
internal static class Program
{
    ...
    private const long MaxAllocatedBytes = 96L * 1024L * 1024L;
    ...
    private static int Main()
    {
        ...
        int checksum = RunWorkload(posix, win32, wsl, wslOut, 64);
        GC.Collect();
        GC.WaitForPendingFinalizers();
        GC.Collect();

        long before = GC.GetAllocatedBytesForCurrentThread();
        checksum += RunWorkload(posix, win32, wsl, wslOut, Iterations);
        ...
        {
            throw new InvalidOperationException("Unity CanonicalPath allocation check exceeded budget: " + allocated + " > " + MaxAllocatedBytes);
        }

        Console.WriteLine("Unity CanonicalPath C# allocation check passed: allocated {0} bytes over {1} iterations", allocated, Iterations);
        return 0;
    }
