...
{
    public struct CanonicalPathValue : IEquatable<CanonicalPathValue>
    {
        private readonly string value;

        public CanonicalPathValue(string value)
        {
            if (string.IsNullOrEmpty(value)) throw new ArgumentException("Canonical path must not be empty.", "value");
            if (value.IndexOf('\0') >= 0) throw new ArgumentException("Canonical path must not contain NUL.", "value");
            this.value = value;
        }
        ...
        public bool Equals(CanonicalPathValue other)
        {
        ...
        {
            if (!(obj is CanonicalPathValue)) return false;
            return Equals((CanonicalPathValue)obj);
        }
    ...
    public sealed class ScopedPathResult
    {
        private readonly UnityMcpPathScope scope;
        private readonly string kind;
        private readonly string path;

        public ScopedPathResult(UnityMcpPathScope scope, string kind, string path)
        {
            this.scope = scope;
            this.kind = kind;
            this.path = path;
        ...
        public UnityMcpPathScope Scope
        {
            get { return scope; }
        }
    ...
    public static class ScopedPathGuard
    {
        public static ScopedPathResult NormalizeScopedPath(UnityMcpPathScope scope, string raw)
        {
            string clean = ValidateScopedRelativeInput(raw);
            if (scope == UnityMcpPathScope.UnityAsset) return ProjectPath(scope, clean, StartsWithAny(clean, new string[] { "Assets", "Packages" }));
            if (scope == UnityMcpPathScope.PackageManifest) return PackageManifestPath(scope, clean);
            if (scope == UnityMcpPathScope.Knowledge) return PrefixedProjectPath(scope, "Assets/UnityMcpKnowledge", clean, IsPlainScopeRelative(clean));
            if (scope == UnityMcpPathScope.Artifact) return PrefixedProjectPath(scope, "Library/SGGUnityMcp", clean, StartsWithAny(clean, new string[] { "job-artifacts", "screenshots" }));
            if (scope == UnityMcpPathScope.GatewayCache)
            {
                if (!StartsWithAny(clean, new string[] { "index" })) throw PathError("ERR_OUTSIDE_ROOT", "gateway cache path must be under index/");
                return new ScopedPathResult(scope, "cache", clean);
            }
            if (scope == UnityMcpPathScope.TempSession) return PrefixedProjectPath(scope, "Temp/SGGUnityMcp", clean, IsPlainScopeRelative(clean));
            throw PathError("ERR_INVALID_PATH", "unsupported Unity MCP path scope");
        }

        public static CanonicalPathValue ToScopedCanonicalPath(ICanonicalPathService service, CanonicalPathValue projectRoot, UnityMcpPathScope scope, string raw)
        {
            if (service == null) throw new ArgumentNullException("service");
            ScopedPathResult result = NormalizeScopedPath(scope, raw);
            if (result.Kind != "project") throw PathError("ERR_INVALID_PATH", "cache scoped paths are not project paths");
            return new CanonicalPathValue(CanonicalPath.Join(projectRoot.Value, result.Path));
        }

        private static string ValidateScopedRelativeInput(string raw)
        {
            if (string.IsNullOrEmpty(raw)) throw PathError("ERR_EMPTY_PATH", "scoped path is empty");
            if (raw.IndexOf('\0') >= 0) throw PathError("ERR_NUL_BYTE", "scoped path contains NUL");
            if (raw.StartsWith("file://", StringComparison.OrdinalIgnoreCase)) throw PathError("ERR_UNSUPPORTED_URI_SCHEME", "file URI is not allowed");
            if (HasEncodedSeparator(raw)) throw PathError("ERR_ENCODED_SEPARATOR", "encoded path separators are not allowed");
            if (raw.StartsWith("/", StringComparison.Ordinal) || raw.StartsWith("\\\\", StringComparison.Ordinal) || HasDriveRoot(raw.Replace('\\', '/'))) throw PathError("ERR_ABSOLUTE_PATH", "scoped path must be relative");
            if (IsDriveRelative(raw)) throw PathError("ERR_DRIVE_RELATIVE_PATH", "drive-relative scoped paths are not allowed");
            if (raw.IndexOf('\\') >= 0) throw PathError("ERR_INVALID_PATH", "scoped paths must use slash separators");

            string[] parts = raw.Split('/');
            for (int i = 0; i < parts.Length; i++)
            {
                string part = parts[i];
                if (part == "..") throw PathError("ERR_OUTSIDE_ROOT", "scoped path traversal is not allowed");
                if (part.Length == 0 || part == "." || part.IndexOf(':') >= 0 || part.EndsWith(".", StringComparison.Ordinal) || part.EndsWith(" ", StringComparison.Ordinal)) throw PathError("ERR_INVALID_PATH", "scoped path contains an invalid component");
                if (part.Length > 255) throw PathError("ERR_INVALID_PATH", "scoped path component exceeds length limit");
            }
            if (raw.Length > 4096) throw PathError("ERR_INVALID_PATH", "scoped path exceeds length limit");
            return raw;
        }

        private static ScopedPathResult ProjectPath(UnityMcpPathScope scope, string path, bool allowed)
        {
            if (!allowed) throw PathError("ERR_OUTSIDE_ROOT", "scoped path is outside its allowed root");
            return new ScopedPathResult(scope, "project", path);
        }

        private static ScopedPathResult PackageManifestPath(UnityMcpPathScope scope, string path)
        {
            if (path.StartsWith("Packages/manifest.json/", StringComparison.Ordinal) || path.StartsWith("Packages/packages-lock.json/", StringComparison.Ordinal)) throw PathError("ERR_INVALID_PATH", "package manifest scope only accepts exact manifest files");
            return ProjectPath(scope, path, path == "Packages/manifest.json" || path == "Packages/packages-lock.json");
        }

        private static ScopedPathResult PrefixedProjectPath(UnityMcpPathScope scope, string root, string path, bool allowed)
        {
            if (!allowed) throw PathError("ERR_OUTSIDE_ROOT", "scoped path is outside its allowed root");
            return new ScopedPathResult(scope, "project", root + "/" + path);
        }

        private static bool IsPlainScopeRelative(string value)
        {
            return !StartsWithAny(value, new string[] {
                "Assets",
                "AssetsEvil",
                "Packages",
                "PackagesEvil",
        ...
        private static bool StartsWithAny(string value, string[] roots)
        {
            for (int i = 0; i < roots.Length; i++)
            {
                string root = roots[i];
                if (value == root || value.StartsWith(root + "/", StringComparison.Ordinal)) return true;
            }
            return false;
        }
        ...
        private static bool HasEncodedSeparator(string value)
        {
            for (int i = 0; i + 2 < value.Length; i++)
            {
                ...
                if (i + 4 < value.Length && high == '2' && low == '5')
                {
                    char decodedHigh = char.ToLowerInvariant(value[i + 3]);
                    char decodedLow = char.ToLowerInvariant(value[i + 4]);
                    if ((decodedHigh == '2' && decodedLow == 'f') || (decodedHigh == '5' && decodedLow == 'c')) return true;
                }
            }
            return false;
        }
        ...
        {
            return value.Length >= 2 && IsAsciiLetter(value[0]) && value[1] == ':' && (value.Length == 2 || (value[2] != '/' && value[2] != '\\'));
        }
    ...
    public sealed class BridgeCanonicalPathService : ICanonicalPathService
    {
        public static readonly BridgeCanonicalPathService Instance = new BridgeCanonicalPathService();

        public CanonicalPathValue Normalize(string input)
        {
            return new CanonicalPathValue(CleanCanonical(input));
        }

        public CanonicalPathValue NormalizeProjectRoot(string input)
        {
            CanonicalPathValue root = Normalize(input);
            if (!IsAbsoluteCanonical(root.Value)) throw new ArgumentException("Unity project root must be absolute.", "input");
            return root;
        }

        public CanonicalPathValue FromUnityAssetPath(CanonicalPathValue projectRoot, string unityPath)
        {
            string cleanUnityPath = PathGuard.NormalizeUnityPath(unityPath);
            return new CanonicalPathValue(Join(projectRoot.Value, cleanUnityPath));
        }

        public string ToUnityAssetPath(CanonicalPathValue projectRoot, CanonicalPathValue fullPath)
        {
            return PathGuard.NormalizeUnityPath(Relative(projectRoot.Value, fullPath.Value));
        }

        public void AssertInsideProject(CanonicalPathValue projectRoot, CanonicalPathValue candidate)
        {
            Relative(projectRoot.Value, candidate.Value);
        }

        public string MakeSafeFileName(string input, int maxLength)
        {
            return PathGuard.MakeSafeFileName(input, maxLength);
        }

        internal static string CleanCanonical(string raw)
        {
            if (string.IsNullOrEmpty(raw)) throw new ArgumentException("Path must not be empty.", "raw");
            if (raw.IndexOf('\0') >= 0) throw new ArgumentException("Path must not contain NUL.", "raw");

            string value = raw.Replace('\\', '/');
            if (IsUriWindowsDrivePath(value)) value = value.Substring(1);
            if (IsDriveRelative(value)) throw new ArgumentException("Windows drive-relative paths are not canonical.", "raw");
            if (HasDriveRoot(value)) value = char.ToLowerInvariant(value[0]) + value.Substring(1);

            string prefix;
            string rest;
            SplitRoot(value, out prefix, out rest);
            ...
            for (int i = 0; i < rawParts.Length; i++)
            {
                ...
                if (part == "..")
                {
                    ...
                    throw new ArgumentException("Relative path escapes above its root.", "raw");
                }
                parts.Add(part);
            }
        ...
        {
            string cleanRelative = PathGuard.NormalizeUnityPath(relative);
            if (root == "/" || root.EndsWith("/", StringComparison.Ordinal)) return root + cleanRelative;
            return root + "/" + cleanRelative;
        }

        internal static string Relative(string root, string target)
        {
            string rootPrefix;
            string rootRest;
            string targetPrefix;
            string targetRest;
            SplitRoot(CleanCanonical(root), out rootPrefix, out rootRest);
            SplitRoot(CleanCanonical(target), out targetPrefix, out targetRest);
        ...
        private static bool IsAbsoluteCanonical(string value)
        {
            return value.StartsWith("/", StringComparison.Ordinal) || HasDriveRoot(value);
        }
        ...
        {
            if (start >= values.Length) return new string[0];
    ...
    {
        public static string NormalizeUnityPath(string unityPath)
        {
            if (string.IsNullOrEmpty(unityPath)) throw new ArgumentException("Unity path must not be empty.", "unityPath");
            if (unityPath.IndexOf('\0') >= 0) throw new ArgumentException("Unity path must not contain NUL.", "unityPath");
            ...
            for (int i = 0; i < rawParts.Length; i++)
            {
                string part = rawParts[i];
                if (part.Length == 0 || part == ".") continue;
                if (part == "..") throw new ArgumentException("Unity path traversal is not allowed.", "unityPath");
                parts.Add(part);
            }
            if (parts.Count == 0) throw new ArgumentException("Unity path must not be empty after cleaning.", "unityPath");

            string clean = string.Join("/", parts.ToArray());
            if (!IsAllowedUnityRoot(clean)) throw new ArgumentException("Unity path must start with Assets/ or Packages/.", "unityPath");
            return clean;
        }

        public static CanonicalPathValue ToCanonicalPath(ICanonicalPathService service, CanonicalPathValue projectRoot, string unityPath)
        {
            if (service == null) throw new ArgumentNullException("service");
            return service.FromUnityAssetPath(projectRoot, unityPath);
        }

        public static void AssertSafeUnityPath(string unityPath)
        {
            NormalizeUnityPath(unityPath);
        }

        public static string MakeSafeFileName(string input, int maxLength)
        {
            if (string.IsNullOrEmpty(input)) throw new ArgumentException("File name input must not be empty.", "input");
            if (input.IndexOf('\0') >= 0) throw new ArgumentException("File name input must not contain NUL.", "input");
            if (maxLength < 1) throw new ArgumentOutOfRangeException("maxLength", "maxLength must be positive.");

            string value = ReplaceUnsafeFileNameChars(input).Trim(' ', '.', '_', '-');
            if (value.Length == 0) value = "file";
            value = EscapeReservedDeviceName(value);
            if (value.Length <= maxLength) return value;
            if (maxLength <= 10) return TrimSafe(value.Substring(0, maxLength));

            string hash = ShortSha256(input);
            string prefix = TrimSafe(value.Substring(0, maxLength - 10));
            return prefix + "--" + hash;
        }

        private static bool IsAbsolutePayload(string value)
        {
        ...
        private static bool IsAllowedUnityRoot(string value)
        {
            return value == "Assets"
        ...
        private static string ReplaceUnsafeFileNameChars(string input)
        {
        ...
        private static string EscapeReservedDeviceName(string value)
        {
            ...
            if (upper == "CON" || upper == "PRN" || upper == "AUX" || upper == "NUL" || IsComOrLpt(upper)) return baseName + "-" + suffix;
            return value;
        }

        private static bool IsComOrLpt(string value)
        {
        ...
        private static string ShortSha256(string input)
        {
            using (SHA256 sha = SHA256.Create())
            {
                ...
                for (int i = 0; i < 4; i++) builder.Append(bytes[i].ToString("x2"));
                return builder.ToString();
            }
        }
        ...
        {
            string trimmed = value.TrimEnd(' ', '.', '_', '-');
    ...
    public static class CanonicalPath
    {
        ...
        public static string Normalize(string raw, CanonicalPathNormalizeOptions options)
        {
            ...
            {
                string mapped = MapWSLDrive(value, options == null ? null : options.WSL);
            ...
            try
            {
                cleaned = BridgeCanonicalPathService.CleanCanonical(value);
            }
        ...
        public static string Relative(string root, string target)
        {
            try
            {
                return BridgeCanonicalPathService.Relative(root, target);
            }
            catch (ArgumentException ex)
            {
        ...
        {
            if (parts == null || parts.Length == 0) throw new ArgumentException("At least one path part is required.", "parts");
            string value = parts[0];
            for (int i = 1; i < parts.Length; i++) value = value.TrimEnd('/') + "/" + parts[i].TrimStart('/', '\\');
            return BridgeCanonicalPathService.CleanCanonical(value);
        }

        public static bool IsEqual(string left, string right, CanonicalPathNormalizeOptions options)
        {
        ...
        public static string ToWSL(string canonical, CanonicalPathWSLOptions options)
        {
            if (canonical == null || canonical.IndexOf('\0') >= 0) throw PathError("ERR_NUL_BYTE", "path contains NUL");
            if (!HasDriveRoot(canonical)) return canonical;
            string mountRoot = options == null || string.IsNullOrEmpty(options.MountRoot) ? "/mnt" : options.MountRoot;
        ...
        private static string ParseFileUri(string uri, CanonicalPathNormalizeOptions options)
        {
            ...
            {
                if (options == null || options.URI == null || !options.URI.AllowFileUri) throw PathError("ERR_UNSUPPORTED_URI_SCHEME", "file URI is not allowed");
                return ParseHierarchicalURIPath(uri, "file://", options);
            }
            if (uri.StartsWith("vscode-file://", StringComparison.Ordinal))
            {
                if (options == null || options.URI == null || !options.URI.AllowVSCodeFileUri) throw PathError("ERR_UNSUPPORTED_URI_SCHEME", "vscode-file URI is not allowed");
        ...
        {
            bool rejectEncodedSlash = options == null || options.URI == null || options.URI.RejectEncodedSlash != false;
        ...
        {
            string rest;
            if (!TrySplitRootRest(value, out rest)) return false;
        ...
        {
            return options == null ? null : options.SourceHost;
        }

        private static string OptionTargetProfile(CanonicalPathNormalizeOptions options)
        {
            return options == null ? null : options.TargetProfile;
        }

        private static bool PreserveExtendedLength(CanonicalPathNormalizeOptions options)
        {
            return options != null && options.Windows != null && options.Windows.PreserveExtendedLength;
        }

        private static bool RejectADS(CanonicalPathNormalizeOptions options)
        {
            return options != null && options.Windows != null && options.Windows.RejectADS;
        }

        private static bool RejectDeviceNames(CanonicalPathNormalizeOptions options)
        {
            return options != null && options.Windows != null && options.Windows.RejectDeviceNames;
        }
