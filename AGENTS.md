...
- The repository is public and GitHub workflows for CI, security baseline, and CodeQL run on `pull_request`, `push` to `main`, and `workflow_dispatch`. Standard public GitHub-hosted Actions are expected to be free; do not add scheduled or unusually heavy workflows without the user asking.
